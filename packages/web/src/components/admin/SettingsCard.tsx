...
const DEADLINE_RE = /^(0[1-9]|1[0-2])-([0-2][0-9]|3[01])$/;
...
export function SettingsCard() {
  ...
  const handleSave = async () => {
    ...
    const valid =
      ...
      contractualDays >= 0 &&
      DEADLINE_RE.test(deadline);
    if (!valid) {
      showError(t('admin.settingsInvalid'));
      return;
    }
    try {
      await updateMutation.mutateAsync({ statutoryDays, contractualDays, carryOverDeadline: deadline });
